...
# 2. 뉴스 수집
# ==========================================
def fetch_news(keywords, start_dt, end_dt):
    all_items = []
    for kw in keywords:
...
def generate_report(api_key, news_data, debug_container):
    debug_container.write("🔄 모델 리스트 확인 중...")
    models = get_available_models(api_key)
    if not models:
        models = ["gemini-1.5-flash", "gemini-1.5-pro", "gemini-1.0-pro"]
        debug_container.warning("⚠️ 모델 리스트 확인 실패 -> 표준 모델로 시도합니다.")
    else:
        debug_container.success(f"✅ 사용 가능 모델: {', '.join(models[:3])}...")
    ...
    **[작성 원칙]**
    1. **서술형 작성**: 단순 나열하지 말고, 현상-원인-전망이 이어지는 논리적 흐름(Narrative)으로 서술하세요.
    ...
    for model in models:
        if "vision" in model or "embedding" in model: continue
        url = f"https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key={api_key}"
        debug_container.markdown(f"<span style='color:gray'>Trying: {model}...</span>", unsafe_allow_html=True)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'candidates' in res_json and res_json['candidates']:
                    ...
                    debug_container.success(f"🎉 성공! ({model})")
                    return True, inject_links_to_report(raw_text, news_data)
                else:
                    debug_container.warning(f"⚠️ {model}: 응답 없음 (Blocked/Empty)")
            elif response.status_code == 429:
                debug_container.warning(f"⏳ {model}: 용량 초과 (429). 2초 대기 후 다음 모델 시도...")
                time.sleep(2)
                continue
            elif response.status_code == 403:
                return False, "API Key 권한 오류 (403). 키를 확인하세요."
        except Exception as e:
            debug_container.error(f"Error ({model}): {str(e)}")
            continue
    return False, "모든 모델 시도 실패. 잠시 후 다시 시도해주세요."
...
if selected_category == "Daily Report":
        ...
        if daily_kws:
            ...
            for i, kw in enumerate(daily_kws):
                if cols[i % 8].button(f"{kw} ×", key=f"del_{kw}"):
                    ...
                    save_keywords(st.session_state.keywords)
                    st.rerun()
        st.caption("⚠️ 키워드 추가/삭제는 즉시 저장됩니다.")
    ...
    if st.button("🚀 리포트 생성 (덮어쓰기)", type="primary", disabled=not bool(api_key)):
        ...
        news_items = fetch_news(daily_kws, start_dt, end_dt)
        ...
        if not news_items:
            debug_box.error("❌ 최종 기사 수집 실패. (키워드 확인 필요)")
        else:
            debug_box.write(f"🧠 AI 분석 시작... (기사 {len(news_items)}건)")
            success, result = generate_report(api_key, news_items, debug_box)
            if success:
                save_data = {'date': target_date_str, 'report': result, 'articles': news_items}
                save_daily_history(save_data)
                debug_box.success("🎉 생성 완료! 화면을 갱신합니다.")
                time.sleep(2)
                st.rerun()
            else:
                debug_box.error(f"🚨 실패: {result}")
