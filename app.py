            ...
            try:
                ...
                <div class="flex justify-between items-center py-2 border-b border-gray-100 hover:bg-gray-50 transition duration-150">
                    ...
                    <div class="text-right">
                        <span class="block text-sm font-bold text-slate-800 font-mono">{fmt_price}</span>
                        <span class="text-xs font-medium {color_class}">{arrow} {sign}{pct:.2f}%</span>
                    </div>
                </div>
                """
    ...
    def replace_match(match):
        try:
            ...
            if 0 <= idx < len(news_data):
                ...
                # Tailwind 스타일 링크
                return f"<a href='{link}' target='_blank' class='text-blue-600 font-bold hover:underline hover:text-blue-800 transition'>[{match.group(1)}]</a>"
        except: pass
...
with st.sidebar:
    ...
    with st.expander("📉 Global Stock (실시간)", expanded=True):
        ...
        if stock_data:
            for cat, items in STOCK_CATEGORIES.items():
                st.markdown(f"<div class='text-xs font-bold text-slate-500 uppercase mt-4 mb-2 tracking-wider'>{cat}</div>", unsafe_allow_html=True)
                for name, symbol in items.items():
    ...
    with st.expander("📂 데이터 백업"):
        kw_json = json.dumps(st.session_state.keywords, ensure_ascii=False, indent=4)
        st.download_button("💾 키워드 다운로드", kw_json, "keywords.json", "application/json", use_container_width=True)
        
        hist_data = load_daily_history()
        if hist_data:
            hist_json = json.dumps(hist_data, ensure_ascii=False, indent=4)
            st.download_button("💾 리포트 다운로드", hist_json, "daily_history.json", "application/json", use_container_width=True)

# 메인 헤더
st.markdown(f"<h1 class='text-3xl font-bold text-slate-800 mb-2'>{selected_category}</h1>", unsafe_allow_html=True)

# ----------------------------------
# [Mode 1] Daily Report
# ----------------------------------
if selected_category == "Daily Report":
    st.markdown("<div class='bg-blue-50 text-blue-800 px-4 py-3 rounded-lg text-sm mb-6'>ℹ️ 매일 오전 6시 기준 반도체 정보 리포트입니다. (전일 12:00 ~ 금일 06:00 수집)</div>", unsafe_allow_html=True)
            ...
            with st.expander(f"{'🔥 ' if is_today else ''}{entry['date']} Daily Report", expanded=is_today):
                ...
                for item in entry.get('articles', []):
                    ...
                    <div class="flex items-start gap-2 mb-1 text-sm text-slate-600">
                        ...
                        <a href="{item['Link']}" target="_blank" class="hover:text-blue-600 hover:underline transition">
                            {item['Title']} <span class="text-xs text-slate-400">({item['Source']})</span>
                        </a>
                    </div>
                    """, unsafe_allow_html=True)
...
# ----------------------------------
    ...
    if data:
        ...
        for item in data:
            # Tailwind 뉴스 카드
            st.markdown(f"""
            <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition duration-200 mb-3">
